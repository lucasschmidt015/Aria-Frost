...
exports.postLogin = (req, res, next) => {
  ...
  User.findOne({ email: email })
    .then((user) => {
      ...
      bcrypt.compare(password, user.password).then((doMetch) => {
        if (!doMetch) {
        ...
        req.session.save((err) => {
          const nextPage = user.verificated
            ? "/"
...
exports.getAccountConfirmationScreen = (req, res, next) => {
  User.findById(req.user._id)
    .then((user) => {
      ...
      return user.updateUserConfirmationToken(user).then((success) => {
...
exports.postReset = (req, res, next) => {
  ...
  User.findOne({ email: email })
    .then((user) => {
      ...
      user.updateResetPasswordToken(user).then((updatedUser) => {
